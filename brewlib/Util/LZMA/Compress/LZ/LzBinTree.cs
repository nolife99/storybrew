...
namespace SevenZip.Compression.LZ
{
	...
	{
        uint _cyclicBufferPos, _cyclicBufferSize, _matchMaxLen, _cutValue = 0xFF, _hashMask, _hashSizeSum, kNumHashDirectBytes, kMinMatchCheck = 4, kFixHashSize = kHash2Size + kHash3Size;
        uint[] _son, _hash;

		bool HASH_ARRAY = true;

		const uint kHash2Size = 1 << 10, kHash3Size = 1 << 16, kBT2HashSize = 1 << 16, kStartMaxLen = 1, kHash3Offset = kHash2Size, kEmptyHashValue = 0, kMaxValForNormalize = ((uint)1 << 31) - 1;
		...
		{
			base.Init();
			for (var i = 0u; i < _hashSizeSum; ++i) _hash[i] = kEmptyHashValue;
			_cyclicBufferPos = 0;
			ReduceOffsets(-1);
		}
		...
		{
			if (historySize > kMaxValForNormalize - 256) throw new ArgumentException(nameof(historySize));
		...
		public uint GetMatches(uint[] distances)
		{
            ...
            uint offset = 0, matchMinPos = (_pos > _cyclicBufferSize) ? (_pos - _cyclicBufferSize) : 0, cur = _bufferOffset + _pos, maxLen = kStartMaxLen, hashValue, hash2Value = 0, hash3Value = 0;
			...
			if (HASH_ARRAY)
			{
				...
				var match = _bufferBase[_bufferOffset + curMatch2] == _bufferBase[cur];
                if (curMatch2 > matchMinPos && match)
				{
                    distances[offset++] = maxLen = 2;
                    distances[offset++] = _pos - curMatch2 - 1;
                }
				if (curMatch3 > matchMinPos && match)
				{
