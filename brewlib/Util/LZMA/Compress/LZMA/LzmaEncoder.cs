...
namespace BrewLib.Util.LZMA.Compress.LZMA
{
    ...
    using BrewLib.Util.LZMA.Compress.RangeCoder;
    ...
    public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
    {
        ...
        static Encoder()
        {
            ...
            for (byte slotFast = 2; slotFast < kFastSlots; ++slotFast)
            {
                ...
                for (var j = 0u; j < k; ++j, ++c) g_FastPos[c] = slotFast;
            }
        }
        ...
        static uint GetPosSlot(uint pos)
        {
            ...
            if (pos < 1 << 21) return (uint)(g_FastPos[pos >> 10] + 20);
            return (uint)(g_FastPos[pos >> 20] + 40);
        }

        static uint GetPosSlot2(uint pos)
        {
            if (pos < 1 << 17) return (uint)(g_FastPos[pos >> 6] + 12);
            if (pos < 1 << 27) return (uint)(g_FastPos[pos >> 16] + 32);
        ...
        void BaseInit()
        {
            _state.Init();
            _previousByte = 0;
            for (var i = 0u; i < Base.kNumRepDistances; ++i) _repDistances[i] = 0;
        }
        ...
        class LiteralEncoder
        {
            ...
            public void Create(int numPosBits, int numPrevBits)
            {
                ...
                for (var i = 0; i < numStates; ++i) m_Coders[i].Create();
            }

            public void Init()
            {
                var numStates = (uint)1 << m_NumPrevBits + m_NumPosBits;
                for (var i = 0; i < numStates; ++i) m_Coders[i].Init();
            }
        ...
        class LenEncoder
        {
            BitEncoder _choice, _choice2;
            readonly BitTreeEncoder[] _lowCoder = new BitTreeEncoder[Base.kNumPosStatesEncodingMax], _midCoder = new BitTreeEncoder[Base.kNumPosStatesEncodingMax];
            readonly BitTreeEncoder _highCoder = new(Base.kNumHighLenBits);
        ...
        {
            readonly uint[] _prices = new uint[Base.kNumLenSymbols << Base.kNumPosStatesBitsEncodingMax], _counters = new uint[Base.kNumPosStatesEncodingMax];
            uint _tableSize;
            ...
            {
                for (var posState = 0u; posState < numPosStates; ++posState) UpdateTable(posState);
            }
        ...
        class Optimal
        {
            ...
            public bool Prev1IsChar, Prev2;

            public uint PosPrev2, BackPrev2, Price, PosPrev, BackPrev, Backs0, Backs1, Backs2, Backs3;
        ...
        readonly BitEncoder[] _isMatch = new BitEncoder[Base.kNumStates << Base.kNumPosStatesBitsMax],
            _isRep = new BitEncoder[Base.kNumStates],
            _isRepG0 = new BitEncoder[Base.kNumStates],
            _isRepG1 = new BitEncoder[Base.kNumStates],
            _isRepG2 = new BitEncoder[Base.kNumStates],
            _isRep0Long = new BitEncoder[Base.kNumStates << Base.kNumPosStatesBitsMax],
            _posEncoders = new BitEncoder[Base.kNumFullDistances - Base.kEndPosModelIndex];

        readonly BitTreeEncoder[] _posSlotEncoder = new BitTreeEncoder[Base.kNumLenToPosStates];
        readonly BitTreeEncoder _posAlignEncoder = new(Base.kNumAlignBits);

        readonly LenPriceTableEncoder _lenEncoder = new(), _repMatchLenEncoder = new();
        readonly LiteralEncoder _literalEncoder = new();

        readonly uint[] _matchDistances = new uint[Base.kMatchMaxLen * 2 + 2];

        uint _numFastBytes = kNumFastBytesDefault, _longestMatchLength, _numDistancePairs, _additionalOffset, _optimumEndIndex, _optimumCurrentIndex;
        bool _longestMatchWasFound;

        readonly uint[] _posSlotPrices = new uint[1 << Base.kNumPosSlotBits + Base.kNumLenToPosStatesBits],
            _distancesPrices = new uint[Base.kNumFullDistances << Base.kNumLenToPosStatesBits],
            _alignPrices = new uint[Base.kAlignTableSize];
        uint _alignPriceCount;

        uint _distTableSize = kDefaultDictionaryLogSize * 2;

        uint _posStateMask = 4 - 1;
        int _posStateBits = 2, _numLiteralPosStateBits, _numLiteralContextBits = 3;

        uint _dictionarySize = 1 << kDefaultDictionaryLogSize, _dictionarySizePrev = 0xFFFFFFFF, _numFastBytesPrev = 0xFFFFFFFF;
        ...
        bool _writeEndMark, _needReleaseMFStream;
        ...
        readonly uint[] reps = new uint[Base.kNumRepDistances], repLens = new uint[Base.kNumRepDistances];
        ...
        uint GetOptimum(uint position, out uint backRes)
        {
            ...
            while (true)
            {
                ...
                var startLen = 2u;
                for (var repIndex = 0u; repIndex < Base.kNumRepDistances; ++repIndex)
                {
        ...
        void WriteEndMarker(uint posState)
        {
            ...
            var posSlot = (1u << Base.kNumPosSlotBits) - 1;
            var lenToPosState = Base.GetLenToPosState(len);
        ...
        public void CodeOneBlock(out long inSize, out long outSize, out bool finished)
        {
            ...
            while (true)
            {
                var len = GetOptimum((uint)nowPos64, out uint pos);
                var posState = (uint)nowPos64 & _posStateMask;
                ...
                else
                {
                    ...
                    if (pos < Base.kNumRepDistances)
                    {
                        ...
                        if (pos != 0)
                        {
                            for (var i = pos; i >= 1; --i) _repDistances[i] = _repDistances[i - 1];
                    ...
                    else
                    {
                        ...
                        var distance = pos;
                        for (var i = Base.kNumRepDistances - 1; i >= 1; --i) _repDistances[i] = _repDistances[i - 1];
                        _repDistances[0] = distance;
                        ++_matchPriceCount;
                    }
        ...
        public void WriteCoderProperties(Stream outStream)
        {
            ...
            for (var i = 0; i < 4; ++i) properties[1 + i] = (byte)(_dictionarySize >> 8 * i & 0xFF);
        ...
        void FillDistancesPrices()
        {
            ...
            for (var lenToPosState = 0u; lenToPosState < Base.kNumLenToPosStates; ++lenToPosState)
            {
                uint posSlot;
        ...
        {
            for (var i = 0u; i < Base.kAlignTableSize; ++i) _alignPrices[i] = _posAlignEncoder.ReverseGetPrice(i);
            _alignPriceCount = 0;
        }
        ...
        {
            for (var m = 0; m < kMatchFinderIDs.Length; ++m) if (s == kMatchFinderIDs[m]) return m;
            return -1;
        }
        ...
        public void SetCoderProperties(CoderPropID[] propIDs, object[] properties)
        {
            for (var i = 0u; i < properties.Length; i++)
            {
                ...
                switch (propIDs[i])
                {
                    ...
                    case CoderPropID.MatchFinder:
                        {
                            ...
                            if (m < 0) throw new ArgumentException(nameof(properties));
                    ...
                    case CoderPropID.DictionarySize:
                        {
                            ...
                            int dicLogSize;
                            for (dicLogSize = 0; dicLogSize < (uint)kDicLogSizeMaxCompress; ++dicLogSize) if (dictionarySize <= (uint)1 << dicLogSize) break;
                    ...
                    default: throw new ArgumentException(nameof(properties));
                }
            }
        }
