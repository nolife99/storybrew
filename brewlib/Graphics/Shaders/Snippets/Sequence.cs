...
public class Sequence(params ShaderSnippet[] snippets) : ShaderSnippet
{
    public override IEnumerable<string> RequiredExtensions
    {
        get
        {
            for (var i = 0; i < snippets.Length; ++i) foreach (var requiredExtension in snippets[i].RequiredExtensions) yield return requiredExtension;
        }
    }
    public override int MinVersion
    {
        get
        {
            ...
            for (var i = 0; i < snippets.Length; ++i) if (snippets[i].MinVersion > minVersion) minVersion = snippets[i].MinVersion;
