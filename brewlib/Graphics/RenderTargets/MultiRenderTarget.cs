...
public class MultiRenderTarget : IDisposable
{
    ...
    void initialize()
    {
        ...
        for (int i = 0, size = renderTextures.Length; i < size; ++i)
        {
            ...
            else if (width != renderTexture.Width || height != renderTexture.Height) throw new InvalidOperationException("Render textures must have the same size");
        ...
        if (depthStencilTexture is not null)
        {
            ...
            switch (depthStencilTexture.Storage)
            {
                ...
                default: throw new NotSupportedException(depthStencilTexture.Storage + " storage isn't supported for the depth / stencil texture.");
            }
        }
        ...
        if (status != FramebufferErrorCode.FramebufferComplete) throw new GraphicsErrorException("frame buffer couldn't be constructed: " + status);
