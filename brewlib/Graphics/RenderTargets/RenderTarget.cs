...
public class RenderTarget(int width, int height, PixelInternalFormat internalFormat, PixelFormat pixelFormat, PixelType pixelType, RenderbufferStorage? renderBufferType = null) : IDisposable
{
    ...
    void initialize()
    {
        ...
        GL.TexImage2D(TextureTarget.Texture2D, 0, internalFormat, width, height, 0, pixelFormat, pixelType, default);
        ...
        if (renderBufferType is not null)
        {
            ...
            switch (renderBufferType.Value)
            {
                ...
                default: throw new NotSupportedException("renderBufferType " + renderBufferType.Value + " isn't supported.");
            }
        }
