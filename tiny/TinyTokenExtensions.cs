...
namespace Tiny
{
    public static class TinyTokenExtensions
    {
        ...
        public static IEnumerable<T> Values<T>(this TinyToken token, object key = null) => token.Value<TinyArray>(key).Values<T>();

        public static T Value<T>(this TinyToken token, object key1, object key2) => token.Value<TinyToken>(key1).Value<T>(key2);
        public static T Value<T>(this TinyToken token, object key1, object key2, object key3) => token.Value<TinyToken>(key1).Value<TinyToken>(key2).Value<T>(key3);
        ...
        public static void Merge(this TinyToken into, TinyToken token)
        {
            ...
            {
                foreach (var entry in tinyArray) intoArray.Add(entry);
            }
