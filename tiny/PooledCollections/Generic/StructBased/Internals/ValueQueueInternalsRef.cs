...
partial class ValueCollectionInternals
{
    ...
    public static ValueQueueInternalsRef<T> GetRef<T>(in ValueQueue<T> source) => new(source);

    /// <summary>Returns the internal array as a <see cref="ReadOnlySpan{T}"/>.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ReadOnlySpan<T> AsReadOnlySpan<T>(in this ValueQueue<T> source, out int head, out int tail)
    {
        head = source._head;
        tail = source._tail;
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ReadOnlyMemory<T> AsReadOnlyMemory<T>(in this ValueQueue<T> source, out int head, out int tail)
    {
        head = source._head;
        tail = source._tail;
