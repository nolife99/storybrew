...
public readonly ref struct TempKVPairDictionary<TKey, TValue>
{
    ...
    public TempKVPairDictionary(in TempDictionary<TKey, TValue> dictionary) => _dictionary = dictionary;
    ...
    public ref struct Enumerator
    {
        ...
        public bool MoveNext()
        {
            ...
            while ((uint)_index < (uint)_dictionary._count)
            {
                ...
                if (entry.Next >= -1)
                {
                    Current = new KVPair<TKey, TValue>(entry.Key, entry.Value);
                    return true;
                }
            }
...
public static class TempDictionaryExtensions
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static TempKVPairDictionary<TKey, TValue> ToTempKVPairDictionary<TKey, TValue>(
        in this TempDictionary<TKey, TValue> dictionary) => new(dictionary);
}