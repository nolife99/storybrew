...
partial class TempCollectionInternalsUnsafe
{
    ...
    public static TempStackInternalsRefUnsafe<T> GetRef<T>(in TempStack<T> source) => new(source);

    /// <summary>Returns the internal array as a <see cref="Span{T}"/>.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Span<T> AsSpan<T>(in this TempStack<T> source) => source._array.AsSpan(0, source._size);

    /// <summary>Returns the internal array as a <see cref="Memory{T}"/>.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static Memory<T> AsMemory<T>(in this TempStack<T> source) => source._array.AsMemory(0, source._size);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static void GetUnsafe<T>(in this TempStack<T> source, out T[] array, out int count)
    {
        array = source._array;
        count = source._size;
    }
