...
partial class TempCollectionInternals
{
    ...
    public static TempDictionaryInternalsRef<TKey, TValue> GetRef<TKey, TValue>(in TempDictionary<TKey, TValue> source)
        => new(source);

    /// <summary>Returns the internal <see cref="Entry{TKey, TValue}"/> array as a <see cref="ReadOnlySpan{T}"/>.</summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static ReadOnlySpan<Entry<TKey, TValue>> AsReadOnlySpan<TKey, TValue>(in this TempDictionary<TKey, TValue> source)
    ...
    public static ReadOnlyMemory<Entry<TKey, TValue>> AsReadOnlyMemory<TKey, TValue>(
        in this TempDictionary<TKey, TValue> source) => source._entries.AsMemory(0, source._count);
}