...
public static class InterpolatingFunctions
{
    ...
    public static readonly Func<float, float, float, float> FloatAngle = (from, to, progress) => from + MathUtil.ShortestAngleDelta(from, to) * progress;
    ...
    public static readonly Func<double, double, float, double> DoubleAngle = (from, to, progress) => from + MathUtil.ShortestAngleDelta(from, to) * progress;
    ...
    public static readonly Func<CommandPosition, CommandPosition, float, CommandPosition> Position = (from, to, progress) => Vector2(from, to, progress);

    ///<summary> Interpolates between 2 <see cref="CommandPosition"/> vectors. </summary>
    public static readonly Func<CommandScale, CommandScale, float, CommandScale> Scale = (from, to, progress) => Vector2(from, to, progress);
    ...
    public static readonly Func<bool, bool, float, bool> BoolFrom = (from, to, progress) => from;

    ///<summary> Returns the second value. </summary>
    public static readonly Func<bool, bool, float, bool> BoolTo = (from, to, progress) => to;

    ///<summary> Returns the OR comparison between the two values. </summary>
    public static readonly Func<bool, bool, float, bool> BoolAny = (from, to, progress) => from || to;

    ///<summary> Returns the AND comparison between the two values. </summary>
    public static readonly Func<bool, bool, float, bool> BoolBoth = (from, to, progress) => from && to;

    ///<summary> Interpolates between 2 <see cref="Storyboarding.CommandValues.CommandColor"/> RGB values. </summary>
    public static readonly Func<CommandColor, CommandColor, float, CommandColor> CommandColor = (from, to, progress) => from + (to - from) * progress;
}