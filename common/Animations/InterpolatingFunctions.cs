...
namespace StorybrewCommon.Animations
{
    ...
    public static class InterpolatingFunctions
    {
        ...
        public static Func<CommandPosition, CommandPosition, double, CommandPosition> Position = (from, to, progress) => new CommandPosition(from.X + (to.X - from.X) * progress, from.Y + (to.Y - from.Y) * progress);
        
        ///<summary> Represents a function that interpolates between 2 <see cref="CommandPosition"/> vectors. </summary>
        public static Func<CommandScale, CommandScale, double, CommandScale> Scale = (from, to, progress) => new CommandScale(from.X + (to.X - from.X) * progress, from.Y + (to.Y - from.Y) * progress);
