...
public class Node3d : Object3d
{
    ...
    ///<summary> The node's rotation keyframes. </summary>
    public readonly KeyframedValue<Quaternion> Rotation = new(Quaternion.Slerp, Quaternion.Identity);

    ///<summary> The node's relative X-scale keyframes of children objects. </summary>
    public readonly KeyframedValue<float> ScaleX = new(float.Lerp, 1);

    ///<summary> The node's relative Y-scale keyframes of children objects. </summary>
    public readonly KeyframedValue<float> ScaleY = new(float.Lerp, 1);

    ///<summary> The node's relative Z-scale keyframes of children objects. </summary>
    public readonly KeyframedValue<float> ScaleZ = new(float.Lerp, 1);

    /// <inheritdoc/>
    public override Matrix4x4 WorldTransformAt(float time)
