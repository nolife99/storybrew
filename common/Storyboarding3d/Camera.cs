...
{
    public readonly float AspectRatio = aspectRatio, FocusDistance = focusDistance, ResolutionScale = resolutionScale,
        NearClip = nearClip, NearFade = nearFade, FarFade = farFade, FarClip = farClip;
    ...
    public static Vector4 ToScreen(Matrix4x4 transform, Vector3 point)
    {
        ...
        var screenPosition = (ndc + Vector2.One) / 2 * new Vector2(OsuHitObject.WidescreenStoryboardSize.Width,
            OsuHitObject.WidescreenStoryboardSize.Height);
...
public class PerspectiveCamera : Camera
{
    ...
    public override CameraState StateAt(float time)
    {
        ...
        if (HorizontalFov.Count > 0)
        {
            ...
            fovY = VerticalFov.Count > 0 ? MathHelper.DegreesToRadians(VerticalFov.ValueAt(time)) : 2 *
        ...
        return new(view * projection, aspectRatio, focusDistance, ResolutionScale, nearClip, nearFade, farFade,
            farClip);
    }
}