...
internal sealed class OsbAnimationWriter(OsbAnimation animation,
    CommandTimeline<CommandPosition> move,
    CommandTimeline<CommandDecimal> moveX,
    CommandTimeline<CommandDecimal> moveY,
    CommandTimeline<CommandDecimal> scale,
    CommandTimeline<CommandScale> scaleVec,
    CommandTimeline<CommandDecimal> rotate,
    CommandTimeline<CommandDecimal> fade,
    CommandTimeline<CommandColor> color,
    TextWriter writer,
    ...
    layer)
{
    ...
    protected override void WriteHeader(OsbSprite sprite, ref readonly StoryboardTransform transform)
    {
        if (sprite is OsbAnimation animation)
        {
            ...
            writer.WriteLine(
                $",{animation.FrameCount},{frameDelay.ToString(exportSettings.NumberFormat)},{animation.LoopType}");
        }
